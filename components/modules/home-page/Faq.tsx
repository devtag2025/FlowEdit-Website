import FaqAccordion from "@/components/sections/FaqAccordion";
import Container from "@/components/shared/Container";
import PageHeaderButton from "@/components/shared/PageHeaderButton";
import Image from "next/image";
import GetStarted from "./GetStarted";

const Faq = () => {
  return (
    <div className="overflow-hidden">
      <div className="w-screen bg-[url('/images/back/07.jpg')] bg-cover bg-center bg-no-repeat">
        {/* <Image
          src="/images/background/gradient-3.png"
          alt="grid"
          fill
          className="object-cover object-top"
        /> */}

        <div className="w-full">
          <Container className="flex flex-col items-center text-center gap-8 sm:gap-10 px-4 py-16">
            <div className="flex flex-col items-center text-center gap-8 sm:gap-10">
              <div className="flex flex-col items-center gap-4 sm:gap-5 max-w-2xl">
                <PageHeaderButton text="FAQs" className="mb-4" />

                <h1 className="font-semibold text-3xl sm:text-4xl md:text-[54px] -tracking-[0.04em] text-black">
                  Frequently asked questions
                </h1>

                <p className="text-sm sm:text-base leading-[150%] text-[rgba(0,0,0,0.7)] max-w-md sm:max-w-lg md:max-w-xl">
                  Find quick answers to common questions and make the most of
                  Maps Explore’s key features, from traffic updates to emergency
                  alerts
                </p>
              </div>

              <FaqAccordion />
            </div>

            <div className="w-full overflow-hidden rounded-2xl pt-[94px] pb-10">
              <GetStarted />
            </div>
          </Container>
        </div>
      </div>
    </div>
  );
};

export default Faq;
